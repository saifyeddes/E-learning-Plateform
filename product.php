    ...
    if (!empty($bundleitemids)) {
        ...
        foreach ($bundleitemids as $bundleitemid) {
            ...
            if (!empty($templatedata['ispurchased'])) {
                ...
                if ($DB->get_manager()->table_exists('elearning_orders') && $DB->record_exists('elearning_orders', [
                    'userid' => (int)$USER->id,
                    'productid' => (int)$bundleproduct->id,
                ])) {
                    $itempurchased = true;
                } else if (!empty($bundleproduct->courseid)) {
                    $coursecontext = context_course::instance((int)$bundleproduct->courseid, IGNORE_MISSING);
                    if ($coursecontext && is_enrolled($coursecontext, (int)$USER->id, '', true)) {
                        $itempurchased = true;
                    }
                }
            }
